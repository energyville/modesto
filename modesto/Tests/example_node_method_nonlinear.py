...
def construct_model():
    ...
    optmodel = Modesto(horizon=n_steps * time_step, time_step=time_step, pipe_model='NodeMethodNonLinear', graph=G)
    ...
    temp_history_return = pd.Series([return_temp] * 30, index=range(30))
    temp_history_supply = pd.Series([supply_temp] * 30, index=range(30))
    mass_flow_history = pd.Series([10] * 30, index=range(30))
    estimated_mf = pd.Series([3] * n_steps, index=range(n_steps))
...
if __name__ == '__main__':
    ...
    optmodel.solve(tee=True, verbose=False, mipgap=0.01, solver='ipopt')
    ...
    fig4, ax4 = plt.subplots()
    ax4.plot(mf['bbThor'], label='bbThor')
    ax4.plot(mf['spWaterschei'], label='spWaterschei')
    ax4.plot(mf['spZwartbergNE'], label='spZwartbergNE')
    ax4.legend()
    ax4.set_title('mass_flows')
    plt.tight_layout()
