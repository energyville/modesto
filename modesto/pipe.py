    ...
    def get_edge_heat(self, node, t, c=None):
        ...
        if c is None:
            if node == self.start_node:
                return -1 * self.get_value('heat_flow_in')[t]
            elif node == self.end_node:
                return self.get_value('heat_flow_out')[t]
            else:
                warnings.warn('Warning: node not contained in this pipe')
                exit(1)
        else:
            if node == self.start_node:
                return -1 * self.get_value('heat_flow_in')[t, c]
            elif node == self.end_node:
                return self.get_value('heat_flow_out')[t, c]
            else:
                warnings.warn('Warning: node not contained in this pipe')
                exit(1)
