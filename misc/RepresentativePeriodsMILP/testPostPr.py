...
for path in ['1dnewsol']: #, '7dnewsol', '3dnewsol']: #  , '7dnewsol']: # ,
    filepath = os.path.join('../RepresentativePeriodsMILP/results_ordered', path)
    for filename in os.listdir(filepath):
        if filename not in ['summary.txt', 'result_ordered4p.txt']:
            ...
            for resultname in ['C_elec']:
                fullname = '{}_full'.format(resultname)
                reprname = '{}_repr'.format(resultname)
                ...
                g.set_axis_labels('Full year {} cost [EUR]'.format(resultname),
                                  'Representative {} cost [EUR]'.format(resultname))
