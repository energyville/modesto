...
test = ResponseFunctionGenerator('sensitivity_supply_temperature_level_280918', horizon=7*24*3600, start_time=pd.Timestamp('20140101'), time_step=900)
...
test.create_sens_cases(['Supply temperature level'])  # , 'Pipe length', 'Pipe diameter', 'Heat demand', 'Supply temperature level',
                         # 'Supply temperature reach', 'Substation temperature difference'
#test.generate_response_functions(tee=False)
test.run_sensitivity_analysis(tee=True)
